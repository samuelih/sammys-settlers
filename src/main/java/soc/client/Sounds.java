...
/**
 * Utility class for creating and playing basic synthesized sounds
 ...
 * to be played later with {@link #toClip(byte[])} or {@link #playPCMBytes(byte[])}.
 *<P>
 ...
 *<P>
 * For cleaner playback on newer platforms like Windows 10,
 * since v2.6.00 you should call {@link #toClip(byte[])} instead of {@link #playPCMBytes(byte[])}.
 * The returned clip can be repeatedly played by calling {@link Clip#setFramePosition(int) Clip.setFramePosition(0)}
 * and then {@link Clip#start()}. To discard each clip and free up its dedicated thread, call {@link Clip#close()}.
 *
 * @since 1.2.00
...
public class Sounds
{
    ...
    /**
     * To reduce latency in {@link #playPCMBytes(byte[])} and {@link #toClip(byte[])},
     * a cached SourceDataLine to try to reopen.
     */
     ...
     * @param vol  Volume (max is 1.0)
     * @return a PCM buffer containing the generated chime,
     *     suitable for {@link #toClip(byte[])} or {@link #playPCMBytes(byte[])}
     ...
     * @param msec  Duration in milliseconds (max is 1000)
     * @param vol  Volume (max is 1.0)
     * @return  A sound buffer, suitable for {@link #toClip(byte[])} or {@link #playPCMBytes(byte[])}
     * @throws IllegalArgumentException if {@code msec} > 1000
    ...
    /**
     * Convert a buffer from {@link #genTone(int, int, double)} or {@link #genChime(int, int, double)}
     * into a higher-level reusable Clip.
     * @param pcmBytes  Buffer to convert; for format details see {@link #genTone(int, int, double, byte[], int)}
     * @return  The clip or {@code null} if {@link LineUnavailableException} or another error occurs
     * @see #playPCMBytes(byte[])
     * @since 2.6.00
     */
    public static final Clip toClip(final byte[] pcmBytes)
    {
        try
        {
        ...
        {
            return null;
            // possibly look into : if (!AudioSystem.isLineSupported(info)) { ... }
        }
    }
